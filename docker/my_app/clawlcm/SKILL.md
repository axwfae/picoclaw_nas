...
> **版本**: v0.8.9 | **更新日**: 2026-04-17 | **基於**: lossless-claw v0.9.1 + lossless-claw-enhanced | **移植工具**: OpenCode + Oh-My-OpenAgent + MiniMax M2.5
...
> **重要**: 每個命令有特定的有效參數，使用錯誤參數會被忽略或報錯。

### 全域參數 (所有命令可用)

| 參數 | 說明 | 可用命令 |
|------|------|---------|
| `--session-key` | 會話鍵 (格式: `user:chat:123`) | bootstrap, ingest, assemble, compact, grep, describe, expand, maintain |
| `--all` | 搜尋所有會話 | grep, describe |
| `--db` | 資料庫路徑 | 所有命令 |
| `--config` | 配置文件路徑 | 所有命令 |
| `--v` | 詳細輸出 | 所有命令 |
...
| 參數 | 說明 | 範例 |
|------|------|------|
| `--session-key` | 會話鍵 (必填) | `user:chat:123` |
| `--session-id` | 會話 ID | `uuid-123` |
| `--token-budget` | Token 預算 | `128000` |
| `--messages` | JSON 訊息陣列 | `'[{"role":"user","content":"你好"}]'` |
...
| `--content` | 訊息內容 (必填) | `你好，世界的定義是什麼？` |
| `--llm-model` | LLM 模型 | `minimax_m2.5` |
| `--llm-base-url` | LLM API 端點 | `http://192.168.1.1:18869` |
...
| 參數 | 說明 | 範例 |
|------|------|------|
| `--session-key` | 會話鍵 | `user:chat:123` |
| `--pattern` | **搜尋模式** (必填) | `Go 語言`, `error` |
| `--mode` | 搜尋模式 | `full_text`, `regex` |
| `--scope` | 搜尋範圍 | `all`, `messages`, `summaries` |
| `--limit` | 結果數量 | `20` |
| `--sort` | 排序方式 | `desc`, `asc` |
| `--all` | 搜尋所有會話 | (不需要值) |
...
|------|------|------|
| `--session-key` | 會話鍵 (必填) | `user:chat:123` |
| `--summary-ids` | 摘要 ID (必填) | `1,2,3` |
| `--query` | 查詢內容 | `什麼是 Go？` |
| `--max-depth` | 最大深度 | `3` |
| `--include-messages` | 包含訊息 | (不需要值) |
...
|------|------|------|
| `--session-key` | 會話鍵 | `user:chat:123` |
| `--maint-op` | 維護操作 | `gc`, `vacuum`, `backup`, `doctor`, `clean`, `rotate` |
| `--all` | 所有會話 | (不需要值) |
...
| `grep` 返回 0 結果 | 可能使用錯誤參數 | 確認使用 `--pattern` 不是 `--query` |
...
> **⚠️ 安全提示**: 請將敏感信息 (apiKey, baseURL) 替換為實際值，不要提交到版本控制
